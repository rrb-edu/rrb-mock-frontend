...
const savedSettings =
  JSON.parse(localStorage.getItem("examSettings"));

let totalTime =
  Number(localStorage.getItem("savedTime")) ||
  (
    savedSettings && savedSettings.duration
      ? Number(savedSettings.duration) * 60
      : 90 * 60
  );
...
const negative =
  settings?.negativeMark
    ? Number(settings.negativeMark)
    : 0.33;
